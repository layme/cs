  ...
  <div v-loading="loading" element-loading-text="后台拼命处理中...">
    <el-form :model="goodsInputDto" ref="goodsInputForm" :rules="rules" class="demo-form-inline" label-width="160px"
      ...
      <el-form-item label="出库数量" prop="number">
        <el-input v-model.number="goodsInputDto.number" placeholder="请在此输入出库数量" clearable></el-input>
      </el-form-item>
      <el-form-item label="备注信息" prop="remark">
        <el-input v-model="goodsInputDto.remark" type="textarea" placeholder="请在此输入备注信息" clearable></el-input>
      </el-form-item>
      <el-form-item align="center">
        <el-button style="width: 150px;" type="warning" icon="el-icon-upload2" @click="submitForm('goodsInputForm')">出 库</el-button>
...
<script>
  export default {
    ...
    methods: {
      ...
      output() {
        this.loading = true;
        this.$http.get('http://localhost:8088/api/stock/out', {params: {gid: this.goodsInputDto.gid,
          number: this.goodsInputDto.number, remark: this.goodsInputDto.remark}, timeout: 3000})
          .then(response => {
            console.log(response);
            this.loading = false;
            if (response.data.result == true) {
              this.message("出库成功", "success");
            } else {
